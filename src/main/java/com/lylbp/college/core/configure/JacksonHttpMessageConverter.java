...
class JacksonHttpMessageConverter extends MappingJackson2HttpMessageConverter {
    ...
    JacksonHttpMessageConverter() {
        getObjectMapper().setSerializerFactory(getObjectMapper().setDateFormat(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).getSerializerFactory().withSerializerModifier(new MyBeanSerializerModifier()));
    }
